...
import { GRAPH_API_ENDPOINTS, DISTRIBUTORSEV_CONTRACT, Network } from './constants';
...
export async function currentDistribution({block, timestamp, network}: {block?: number, timestamp?: number, network: Network} = {network: 'mainnet'}) {
    ...
    const result = await request(GRAPH_API_ENDPOINTS[network].distributionSEV,
        gql`{
                distributor(id: "${DISTRIBUTORSEV_CONTRACT[network]?.address}", ${condition}) {
                    currentDistribution {
...
export async function distributions({block, timestamp, network}: {block?: number, timestamp?: number, network: Network} = {network: 'mainnet'}) {
    const promise = graphResultsPager({
        api: GRAPH_API_ENDPOINTS[network].distributionSEV,
        query: {
...
export async function debtTotal({block, timestamp, network}: {block?: number, timestamp?: number, network: Network} = {network: 'mainnet'}) {
    const condition = await makeCodition({block, timestamp, network});
    ...
    return result.distributor?.debtTotal
        ? BigInt(result.distributor?.debtTotal)
        : BigInt(0);
}
...
const distributionProperties = {
    ...
    callback(results: Distribution[]) {
        return results.map(distribution => { return {
            ...
            tokenTotal: BigInt(distribution.tokenTotal)
        }});
    }
}
